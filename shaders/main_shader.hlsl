...
HSConstants HS_Constants(InputPatch<ControlPoint, 3> patch, uint patchId : SV_PrimitiveID)
{
    ...
    float tessFactor = max(1.0, min(8.0, lerp(8.0, 1.0, saturate((distanceToCamera - 5.0) / 45.0))));
...
PixelIn DS_Main(HSConstants hsc, float3 bary : SV_DomainLocation, const OutputPatch<ControlPoint, 3> patch)
{
    ...
    float displacement = gDisplacementMap.SampleLevel(gLinearWrap, texC, 0.0).r;
    float dispStrength = (gObjectPadding.y > 0.0f) ? gObjectPadding.y : 0.4f;

    if (gObjectPadding.x >= 2.5f)
    {
        // F3 debug: for "height_darker" maps treat darker areas as higher.
...
GBufferOut PS_Geometry(PixelIn pin)
{
    ...
    {
        // F2 debug: show world normal as color.
        albedo = float4(normalW * 0.5f + 0.5f, 1.0f);
    }
    else if (gObjectPadding.x >= 2.5f)
    {
        // F3 debug: show displacement texture as grayscale.
        float d = gDisplacementMap.Sample(gLinearWrap, pin.TexC).r;
        d = 1.0f - d;
        albedo = float4(d, d, d, 1.0f);
    }

    pout.Albedo = albedo;
    pout.Normal = float4(normalW * 0.5f + 0.5f, 1.0f);
    // SV_POSITION.z in pixel shader is already post-projection depth in [0..1].
